...
LoudnessResult measure_loudness(const std::filesystem::path& audio_file)
{
    ...
    result.success = true;
    spdlog::info("Loudness: {:.1f} LUFS, LRA: {:.1f} LU, Peak: {:.1f} dBTP",
                 result.integrated_lufs, result.loudness_range_lu, result.true_peak_dbtp);
    return result;
}
...
NormalizeResult normalize_loudness(const NormalizeOptions& opts)
{
    ...
    std::string cmd = "ffmpeg -y -i " + opts.input_file.string() +
                      ...
                      ":TP=" + std::to_string(opts.true_peak_limit) +
                      ":LRA=11 " + opts.output_file.string() + " 2>&1";
