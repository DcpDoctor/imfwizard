...
static void write_dcp_cpl(const std::filesystem::path& path,
                          const std::string& cpl_uuid,
                          const std::string& title,
                          const std::string& content_kind,
                          const std::string& video_uuid,
...
DcpConvertResult convert_imp_to_dcp(const DcpConvertOptions& opts)
{
  ...
  write_dcp_cpl(opts.output_dir / "CPL.xml", result.cpl_uuid, title,
                opts.content_kind, video_uuid, audio_uuid, frame_count, 24, 1);
  ...
  // Write ASSETMAP
  {
    ...
    f << "  <AssetList>\n";
    f << "    <Asset><Id>urn:uuid:" << result.cpl_uuid << "</Id><ChunkList><Chunk><Path>CPL.xml</Path></Chunk></ChunkList></Asset>\n";
    f << "    <Asset><Id>urn:uuid:" << video_uuid << "</Id><ChunkList><Chunk><Path>" << video_uuid << ".mxf</Path></Chunk></ChunkList></Asset>\n";
    if(!audio_uuid.empty())
      f << "    <Asset><Id>urn:uuid:" << audio_uuid << "</Id><ChunkList><Chunk><Path>" << audio_uuid << ".mxf</Path></Chunk></ChunkList></Asset>\n";
    f << "  </AssetList>\n";
    f << "</AssetMap>\n";
  }
