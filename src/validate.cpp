...
ValidationResult validate_with_photon(const std::filesystem::path& imp_dir)
{
    ...
    if (!photon_available()) {
        result.notes.push_back({
            ValidationNote::Severity::warning,
            "Java not found — cannot run Photon validator",
            ""
        });
        return result;
    }
    ...
    std::string cmd = "java -jar " + photon_jar + " -i " +
                      imp_dir.string() + " 2>&1";
    ...
    spdlog::info("Photon validation {}: {} errors, {} warnings",
