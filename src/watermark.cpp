...
static void embed_in_frame(const std::filesystem::path& input, const std::filesystem::path& output,
                           const std::vector<uint8_t>& bits, uint8_t strength)
{
...
WatermarkResult apply_watermark(const WatermarkOptions& opts)
{
  ...
  spdlog::info("Watermarking {} ({} payload bits, strength {})", opts.input_dir.string(),
               bits.size(), opts.strength);
