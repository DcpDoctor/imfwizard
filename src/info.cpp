...
static fs::path find_pkl(const fs::path& dir)
{
    for (const auto& entry : fs::directory_iterator(dir)) {
        if (entry.path().filename().string().starts_with("PKL_") &&
...
{
    for (const auto& entry : fs::directory_iterator(dir)) {
        if (entry.path().filename().string().starts_with("CPL_") &&
            entry.path().extension() == ".xml") {
            return entry.path();
        }
    }
    ...
    try {
        ...
        if (!pkl_path.empty()) {
            ...
            if (pkl_doc) {
                ...
                if (id_node) info.pkl_uuid = get_text(id_node);
                xmlNodePtr issuer_node = find_child(root, "Issuer");
                if (issuer_node) info.issuer = get_text(issuer_node);
                xmlNodePtr date_node = find_child(root, "IssueDate");
                if (date_node) info.issue_date = get_text(date_node);
                ...
                if (asset_list) {
                    for (xmlNodePtr asset = asset_list->children; asset; asset = asset->next) {
                        ...
                        if (uuid_node) track.uuid = get_text(uuid_node);
                        xmlNodePtr fn_node = find_child(asset, "OriginalFileName");
                        if (fn_node) track.filename = get_text(fn_node);
                        xmlNodePtr type_node = find_child(asset, "Type");
                        if (type_node) {
                            std::string mime = get_text(type_node);
                            if (mime.find("mxf") != std::string::npos) track.type = "essence";
                            else track.type = "metadata";
                        }
                        xmlNodePtr size_node = find_child(asset, "Size");
                        if (size_node) track.size = std::stoull(get_text(size_node));
        ...
        if (!cpl_path.empty()) {
            ...
            if (cpl_doc) {
                ...
                xmlNodePtr id_node = find_child(root, "Id");
                if (id_node) info.cpl_uuid = get_text(id_node);
                xmlNodePtr title_node = find_child(root, "ContentTitle");
                if (title_node) info.cpl_title = get_text(title_node);
                xmlFreeDoc(cpl_doc);
            }
        }
