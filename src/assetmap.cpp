...
void generate_assetmap(const AssetMapOptions& opts, const std::filesystem::path& output_dir)
{
  ...
  for(const auto& entry : opts.assets)
  {
    ...
    xmlNewChild(chunk, nullptr, BAD_CAST "Path", BAD_CAST entry.path.string().c_str());
    xmlNewChild(chunk, nullptr, BAD_CAST "Length", BAD_CAST std::to_string(entry.size).c_str());
  }
