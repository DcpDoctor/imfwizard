...
static MxfTrackFile wrap_j2k(const WrapOptions& opts)
{
  ...
  spdlog::info("J2K sequence: {}x{}, {} frames", pdesc.StoredWidth, pdesc.StoredHeight,
  ...
  result = writer.OpenWrite(opts.output_path.string(), info, essence_descriptor,
                            essence_sub_descriptors, edit_rate, 16384, AS_02::IS_FOLLOW, 10);
  if(ASDCP_FAILURE(result))
...
static MxfTrackFile wrap_pcm(const WrapOptions& opts)
{
  ...
  result = writer.OpenWrite(opts.output_path.string(), info, wave_desc, essence_sub_descriptors,
                            edit_rate, 16384);
  if(ASDCP_FAILURE(result))
