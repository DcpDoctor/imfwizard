...
std::vector<EdlEntry> parse_edl(const std::filesystem::path& edl_file)
{
    ...
    std::string line;
    std::regex event_re(R"(^\s*(\d+)\s+(\S+)\s+([VAB]\d*)\s+\w+\s+(\d{2}:\d{2}:\d{2}:\d{2})\s+(\d{2}:\d{2}:\d{2}:\d{2})\s+(\d{2}:\d{2}:\d{2}:\d{2})\s+(\d{2}:\d{2}:\d{2}:\d{2}))");
...
ConformResult conform_from_edl(const ConformOptions& opts)
{
    ...
    for (const auto& e : result.entries) {
        if (e.record_out > max_out) max_out = e.record_out;
    }
    ...
    for (const auto& entry : result.entries) {
        if (entry.track_type[0] != 'V') continue;
