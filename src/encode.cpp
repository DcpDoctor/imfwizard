...
ImageFormat detect_format(const fs::path& file)
{
    ...
    if (ext == ".dpx") return ImageFormat::DPX;
    ...
    if (ext == ".j2c" || ext == ".j2k" || ext == ".jp2") return ImageFormat::J2K;
...
ImageFormat detect_sequence_format(const fs::path& dir)
{
    for (const auto& entry : fs::directory_iterator(dir)) {
        ...
        if (fmt != ImageFormat::Unknown) return fmt;
    }
    return ImageFormat::Unknown;
}
...
uint32_t count_frames(const fs::path& dir)
{
    ...
    for (const auto& entry : fs::directory_iterator(dir)) {
        if (!entry.is_regular_file()) continue;
        auto fmt = detect_format(entry.path());
        if (fmt != ImageFormat::Unknown) ++count;
    }
    return count;
}
...
EncodeResult encode_to_j2k(const EncodeOptions& opts)
{
    ...
    spdlog::info("Encoding {} frames ({}) -> J2K", result.frame_count,
                 format_to_input_flag(fmt));
    ...
    for (const auto& input_file : input_files) {
        ...
        if (encoder == "grk_compress") {
            cmd = "grk_compress"
                  " -i " + input_file.string() +
                  " -o " + out_path.string() +
                  " -r " + std::to_string(opts.target_bitrate_mbps) +
                  " -n " + std::to_string(opts.num_resolutions) +
                  " -b " + std::to_string(opts.code_block_width) + "," +
