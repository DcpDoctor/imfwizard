...
DeliveryProfile netflix_profile()
{
    ...
    p.max_width = 3840; p.max_height = 2160;
...
DeliveryProfile cinema_2k_profile()
{
    ...
    p.max_width = 2048; p.max_height = 1080;
    p.max_bitrate_mbps = 250;
    p.bit_depth = 12;
...
DeliveryProfile cinema_4k_profile()
{
    ...
    p.max_width = 4096; p.max_height = 2160;
...
DeliveryProfile broadcast_hd_profile()
{
    ...
    p.max_width = 1920; p.max_height = 1080;
...
DeliveryProfile archival_profile()
{
    ...
    p.max_width = 0; p.max_height = 0; // No limit
...
{
    if (platform == "netflix") return netflix_profile();
    if (platform == "disney") return disney_profile();
    if (platform == "amazon") return amazon_profile();
    if (platform == "apple") return apple_profile();
    if (platform == "cinema-2k") return cinema_2k_profile();
    if (platform == "cinema-4k") return cinema_4k_profile();
    if (platform == "broadcast-hd") return broadcast_hd_profile();
    if (platform == "broadcast-uhd") return broadcast_uhd_profile();
    if (platform == "archival") return archival_profile();
